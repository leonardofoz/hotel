...
class CidadeController extends AppBaseController
{
	...
	public function Query()
	{
		try
		{
			...
			if ($filter) $criteria->AddFilter(
				new CriteriaFilter('Id,Nome,Estado'
				, '%'.$filter.'%')
			);
			...
			if ($page != '')
			{
				...
				$cidades = $this->Phreezer->Query('CidadeReporter',$criteria)->GetDataPage($page, $pagesize);
			...
			{
				// return all results
				$cidades = $this->Phreezer->Query('CidadeReporter',$criteria);
				$output->rows = $cidades->ToObjectArray(true, $this->SimpleObjectParams());
	...
	public function Create()
	{
		try
		{
			...
			$cidade->Estado = $this->SafeGetVal($json, 'estado');
	...
	public function Update()
	{
		try
		{
			...
			$cidade->Estado = $this->SafeGetVal($json, 'estado', $cidade->Estado);
